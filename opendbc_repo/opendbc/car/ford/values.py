...
class CarControllerParams:
  ...
  CURVATURE_LIMITS: CurvatureSteeringLimits = CurvatureSteeringLimits(0.02)
...
class RADAR:
  ...
  #
  # This is NOT a DBC filename.
  # RadarInterface handles MR76 directly from raw CAN frames.
  MR76 = 'MR76'
...
class CAR(Platforms):
  ...
  FORD_ESCAPE_MK4 = FordPlatformConfig(
    [
      ...
      FordCarDocs("Ford Kuga 2020-23", "Adaptive Cruise Control with Lane Centering",
                  hybrid=True, plug_in_hybrid=True),
    ],
  ...
  FORD_ESCAPE_MK4_5 = FordCANFDPlatformConfig(
    [
      FordCarDocs("Ford Escape 2023-24", hybrid=True, plug_in_hybrid=True,
                  setup_video="https://www.youtube.com/watch?v=M6uXf4b2SHM"),
  ...
  FORD_EXPLORER_MK6 = FordPlatformConfig(
    [
      FordCarDocs("Ford Explorer 2020-24", hybrid=True),
      FordCarDocs("Lincoln Aviator 2020-24", "Co-Pilot360 Plus", plug_in_hybrid=True),
    ],
  ...
  FORD_FOCUS_MK4 = FordPlatformConfig(
    [FordCarDocs("Ford Focus 2018-22", "Adaptive Cruise Control with Lane Centering",
                 footnotes=[Footnote.FOCUS], hybrid=True)],
  ...
  FORD_MUSTANG_MACH_E_MK1 = FordCANFDPlatformConfig(
    [FordCarDocs("Ford Mustang Mach-E 2021-24", "All",
                 setup_video="https://www.youtube.com/watch?v=AR4_eTF3b_A")],
    CarSpecs(mass=2200, wheelbase=2.984, steerRatio=17.0),
  )

  FORD_RANGER_MK2 = FordCANFDPlatformConfig(
    [FordCarDocs("Ford Ranger 2024", "Adaptive Cruise Control with Lane Centering",
                 setup_video="https://www.youtube.com/watch?v=2oJlXCKYOy0")],
...
def match_fw_to_car_fuzzy(
  live_fw_versions: LiveFwVersions,
  vin: str,
  offline_fw_versions: OfflineFwVersions,
) -> set[str]:
  ...
  for candidate, fws in offline_fw_versions.items():
    ...
    for ecu, expected_versions in fws.items():
      ...
      expected_model_year_hints = {
        model_year_hint
        for _, model_year_hint in codes
      }

      codes = get_platform_codes(
        live_fw_versions.get(addr, set())
      )

      found_platform_codes = {
        code
        for code, _ in codes
      }

      found_model_year_hints = {
        model_year_hint
        for _, model_year_hint in codes
      }
      ...
      if not any(
        min(expected_model_year_hints)
        <= found_model_year_hint
        <= max(expected_model_year_hints)
        for found_model_year_hint in found_model_year_hints
      ):
        break
...
def ford_asbuilt_block_response(block_id: int):
  return (
    bytes([uds.SERVICE_TYPE.READ_DATA_BY_IDENTIFIER + 0x40])
    + p16(DATA_IDENTIFIER_FORD_ASBUILT + block_id - 1)
  )
...
FW_QUERY_CONFIG = FwQueryConfig(
  fw_version_regex=(
    br"[A-HJ-NP-VX-Z][0-9A-HJ-NP-VX-Z]{3}-"
    br"[0-9A-HJ-NP-VX-Z]{5,6}-"
    br"[A-HJ-NP-VX-Z]{2,}\x00*"
  ),
  ...
  requests=[
    ...
    *[
      Request(
        [
          StdQueries.TESTER_PRESENT_REQUEST,
          ford_asbuilt_block_request(block_id),
        ],
        [
          StdQueries.TESTER_PRESENT_RESPONSE,
          ford_asbuilt_block_response(block_id),
        ],
        whitelist_ecus=ecus,
        bus=0,
        logging=True,
